...
type
  ...
  public
    ...
    [Test] procedure Schema_Default_String_Value;
    [Test] procedure Schema_Default_Integer_Value;
  end;
...
procedure AssertArrayDoesNotContain(const Arr: TJSONArray; const Value: string);
var
  V: TJSONValue;
begin
  for V in Arr do
    if SameText(V.Value, Value) then
      Assert.Fail(Format('Did not expect array to contain "%s"', [Value]));
end;
...
[Test]
procedure TPasDanticValidatorTests.Schema_Default_String_Value;
var
  Schema, Props, RoleProp: TJSONObject;
  Required: TJSONArray;
begin
  Schema := GenerateJSONSchema(TDefaultsModel);
  try
    Props := GetObj(Schema, 'properties');
    RoleProp := GetObj(Props, 'Role');

    Assert.AreEqual('string', RoleProp.GetValue<string>('type'));
    Assert.AreEqual('guest', RoleProp.GetValue<string>('default'));

    Required := GetArr(Schema, 'required');
    AssertArrayDoesNotContain(Required, 'Role');
  finally
    Schema.Free;
  end;
end;
...
[Test]
procedure TPasDanticValidatorTests.Schema_Default_Integer_Value;
var
  Schema, Props, AgeProp: TJSONObject;
  Required: TJSONArray;
begin
  ...
  try
    ...
    Assert.AreEqual('integer', AgeProp.GetValue<string>('type'));
    Assert.AreEqual(30, AgeProp.GetValue<Integer>('default'));

    Required := GetArr(Schema, 'required');
    AssertArrayDoesNotContain(Required, 'Age');
  finally
    Schema.Free;
  end;
end;
